...
import {
    ...
    Clock, MapPin, Briefcase, ChevronRight, Activity, Percent, ChevronDown, ChevronUp
} from 'lucide-react';
...
const MeasurementReport = () => {
    ...
    const GENDER_DATA = [
        { name: 'Male', value: campaign.audience.gender.male, color: '#3b82f6' },
        { name: 'Female', value: campaign.audience.gender.female, color: '#ec4899' }
    ];
        ...
        <div style={{ padding: '24px' }}>
            ...
            <div className="dashboard-grid" style={{ marginBottom: '32px' }}>
                <div className="card">
                    ...
                    <div className="stat-val">{(campaign.impressions / 1000000).toFixed(2)}M</div>
                ...
                <div className="card">
                    ...
                    <div className="stat-val">{(campaign.reach / 1000).toFixed(0)}k</div>
                ...
                <div className="card">
                    ...
                    <div className="stat-val">{campaign.frequency}x</div>
                ...
                <div className="card">
                    ...
                    <div className="stat-val">RM {campaign.cpm.toFixed(2)}</div>
                ...
                <div className="card" style={{ marginBottom: '32px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '16px', marginBottom: '24px' }}>
                        <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', textAlign: 'center' }}>
                            ...
                            <div style={{ fontSize: '20px', fontWeight: 'bold' }}>{(campaign.delivery.bidRequests / 1000000).toFixed(2)}M</div>
                        </div>
                        <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', textAlign: 'center' }}>
                            <div style={{ fontSize: '11px', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>Bid Responses</div>
                            <div style={{ fontSize: '20px', fontWeight: 'bold' }}>{(campaign.delivery.bidResponses / 1000000).toFixed(2)}M</div>
                        </div>
                        <div style={{ background: '#f0fdf4', padding: '16px', borderRadius: '12px', textAlign: 'center' }}>
                            ...
                            <div style={{ fontSize: '20px', fontWeight: 'bold', color: '#10b981' }}>{(campaign.delivery.bidsWon / 1000000).toFixed(2)}M</div>
                        </div>
                        <div style={{ background: '#eff6ff', padding: '16px', borderRadius: '12px', textAlign: 'center' }}>
                            ...
                            <div style={{ fontSize: '20px', fontWeight: 'bold', color: '#3b82f6' }}>{campaign.delivery.fillRate}%</div>
                        </div>
                        <div style={{ background: '#fef3c7', padding: '16px', borderRadius: '12px', textAlign: 'center' }}>
                            ...
                            <div style={{ fontSize: '20px', fontWeight: 'bold', color: '#f59e0b' }}>{campaign.delivery.winRate}%</div>
                        </div>
                        <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '12px', textAlign: 'center' }}>
                            <div style={{ fontSize: '11px', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>Avg Latency</div>
                            <div style={{ fontSize: '20px', fontWeight: 'bold' }}>{campaign.delivery.avgLatency}ms</div>
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginTop: '24px', paddingTop: '24px', borderTop: '1px solid var(--border)' }}>
                        <div style={{ textAlign: 'center' }}>
                            ...
                            <div style={{ fontSize: '16px', fontWeight: 'bold' }}>RM {campaign.delivery.avgBidPrice.toFixed(2)}</div>
                        </div>
                        <div style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: '11px', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Avg Win Price</div>
                            <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#10b981' }}>RM {campaign.delivery.avgWinPrice.toFixed(2)}</div>
                        </div>
                        <div style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: '11px', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Bid Floor</div>
                            <div style={{ fontSize: '16px', fontWeight: 'bold' }}>RM {campaign.delivery.bidFloor.toFixed(2)}</div>
                        </div>
                        <div style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: '11px', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Timeouts / Errors</div>
                            <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#ef4444' }}>{(campaign.delivery.timeouts / 1000).toFixed(1)}k / {(campaign.delivery.errors / 1000).toFixed(1)}k</div>
                        </div>
                    </div>
                </div>
            )}
                            ...
                            {campaign.assets.map(asset => (
                                <React.Fragment key={asset.id}>
                                    <tr style={{ borderBottom: '1px solid #f1f5f9', cursor: 'pointer' }} onClick={() => setExpandedAsset(expandedAsset === asset.id ? null : asset.id)}>
                                        ...
                                        <td style={{ padding: '16px 20px', textAlign: 'right', fontSize: '14px' }}>{asset.frequency.toFixed(2)}x</td>
                                        <td style={{ padding: '16px 20px', textAlign: 'right', fontSize: '14px', fontWeight: 'bold' }}>RM {asset.cpm.toFixed(2)}</td>
